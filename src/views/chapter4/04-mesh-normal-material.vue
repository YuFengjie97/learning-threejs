...
/**
 * normal材质属性shading已经被弃用，
 * 文档里只有一个flatShading，修改后没有发现前后渲染效果有什么不同
 */
...
let meshMaterial: THREE.MeshNormalMaterial = new THREE.MeshNormalMaterial() // 全局材质，cube，sphere，plane共用材质
...
function initGUI() {
  ...
  meshGui.add(mesh, 'visible').onChange(val=>{
    cube.visible = val
    sphere.visible = val
    plane.visible = val
  })
  meshGui.add(mesh, 'flatShading').onChange(val=>{
    meshMaterial.flatShading = val
  ...
  wireframeGui
    .add(mesh, 'wireframeLinewidth', 0, 10, 0.1)
    .name('lineWidth')
    .onChange((val) => {
