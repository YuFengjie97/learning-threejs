...
function initGUI() {
  ...
  folder.open()
  folder.addColor(pointLightParams, 'color').onChange((val) => {
    pointLight.color = new THREE.Color(val)
  })
  folder.add(pointLightParams, 'intensity', 0, 4, 0.1).onChange((val) => {
    pointLight.intensity = val
  })
  folder.add(pointLightParams, 'distance', 0, 400, 1).onChange((val) => {
    pointLight.distance = val
  })
  folder.add(pointLightParams, 'decay', 0, 2, 0.1).onChange((val) => {
...
function initLight() {
  ...
  pointLightHelper = new THREE.PointLightHelper(pointLight, 4)
...
function animate() {
  ...
  pointLight.position.copy(targetMesh.position)
}
// 绘制
function render() {
