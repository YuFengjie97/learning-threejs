...
function initGUI() {
  ...
  folder.open()
  folder.add(lightParams, 'stopMoving')
  folder.add(lightParams, 'showHelper').onChange(val => {
    lightHelper.visible = val
  })
  folder.add(lightParams, 'castShadow').onChange(val=>{
    light.castShadow = val
  })
  folder
    .add(lightParams, 'target', ['plane', 'cube', 'sphere'])
    .onChange((val) => {
      if (val === 'plane') light.target = planeMesh
      else if (val === 'cube') light.target = cubeMesh
      else if (val === 'sphere') light.target = sphereMesh
    })
  folder.addColor(lightParams, 'color').onChange((val) => {
    light.color = new THREE.Color(val)
  })
  folder.add(lightParams, 'intensity', 0, 4, 0.1).onChange((val) => {
    light.intensity = val
  })
}
...
function initLight() {
  ...
  const { color, intensity, distance, decay } = lightParams
...
function animate() {
  ...
  // 小球与点光源位置更新
  if(!lightParams.stopMoving){
    let { x, y } = getTargetPos()
    targetMesh.position.set(x, 4, y)
    light.position.copy(targetMesh.position)
