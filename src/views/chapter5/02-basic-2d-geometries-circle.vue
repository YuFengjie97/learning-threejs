...
function initGUI() {
  ...
  gui.add(params, 'radius', 0, 20, 1).onChange((val) => {
    scene.remove(mesh)
    createMesh()
  })
  gui.add(params, 'segments', 0, 40, 1).onChange((val) => {
    scene.remove(mesh)
    createMesh()
  })
  gui.add(params, 'thetaStart', 0, PI*2, 0.01).onChange((val) => {
    scene.remove(mesh)
    createMesh()
  })
  gui.add(params, 'thetaLength', 0, PI*2, 0.01).onChange((val) => {
    scene.remove(mesh)
    createMesh()
  })
}
...
function createMesh() {
  ...
  const geo = new THREE.CircleGeometry(radius, segments, thetaStart, thetaLength)
